...
//    }
    ...
    private function addOperation(string $targetPath, string $templateName, array $variables): void
    {
        ...
        if (!file_exists($templatePath)) {
            $templatePath = \sprintf('%s/templates/%s', \dirname(__DIR__, 2), $templateName);

            if (!file_exists($templatePath)) {
                $templatePath = \sprintf('%s/vendor/symfony/maker-bundle/templates/%s', \dirname(__DIR__, 2), $templateName);
            }
